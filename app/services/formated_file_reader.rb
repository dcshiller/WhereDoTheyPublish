class FormatedFileReader
  TITLES = %w[ de van von le De Van Von Le ]
  attr_reader :journal_name, :divider

  FILE = "data/australasian_journal_of_professional_and_applied_ethics"
  JOURNAL = Journal.find_by(name: "Australasian Journal of Professional and Applied Ethics")
  ...
  def self.read
    ...
    lines.each do |line|
      ...
      if line.starts_with? "Volume"
        ...
        pubs << Publication.create(title: title, journal: JOURNAL, authors: authors, publication_year: @year, volume: @volume, number: @number, categorization: {})
      end
    end
  end
  # 
